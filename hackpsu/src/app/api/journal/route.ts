...
async function checkAndAwardBadges(supabase: any, userId: string, entryText: string) {
    ...
    // Award "First Steps" badge if this is the first entry
    if (entryCount === 1) {
    ...
    // Check for "Wellness Warrior" (30 entries)
    if (entryCount === 30) {
        const { data: wellnessBadge } = await supabase
            .from('badges')
            .select('id')
            .eq('name', 'Wellness Warrior')
            .single();
    ...
    // Check for "Reflection Master" (100 entries)
    if (entryCount === 100) {
        const { data: reflectionBadge } = await supabase
            .from('badges')
            .select('id')
            .eq('name', 'Reflection Master')
            .single();
        ...
        if (reflectionBadge) {
            const { data: existingBadge } = await supabase
                ...
                .select('id')
                .eq('user_id', userId)
                .eq('badge_id', reflectionBadge.id)
                .single();
    ...
    if (currentHour < 7) {
        const { data: earlyBirdBadge } = await supabase
            .from('badges')
            .select('id')
            .eq('name', 'Early Bird')
            .single();
        ...
        if (earlyBirdBadge) {
            ...
            if (!existingBadge) {
                await supabase
                    ...
                    .insert({
                        user_id: userId,
                        badge_id: earlyBirdBadge.id
                    });
            }
        }
    }

    // Midnight Owl (after 10 PM)
    if (currentHour >= 22) {
        const { data: owlBadge } = await supabase
            .from('badges')
            .select('id')
            .eq('name', 'Midnight Owl')
            .single();
